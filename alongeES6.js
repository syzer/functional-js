...
const truthyAll = ([first, ...rest]) =>
    first === undefined ? [] : [Boolean(first), ...truthyAll(rest)]

log(truthyAll([null, true, 25, false, 'foo']))
// =>[ false, true, true, false, true ]

log(map(x => Boolean(x), [null, true, 25, false, 'foo']))
// => [false,true,true,false,true]
...
const {name: {first: given, last: surname}, occupation: {title}} = user
const descr = ({name: {first: given}, occupation: {title}}) =>
    `${given} is a ${title}`
