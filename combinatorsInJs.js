...
const curriedArrow = curry(arrow)
print(curriedArrow('basel', 'zurich')) // => basel -> zurich

const fromBaselTo = curriedArrow('basel')
...
const mixFirst = before(function () {
    this.mix()
})
...
const around = curry(
    (decoration, method) => {
        return function decoratedWithAround() {
            const methodPrepended = [method].concat(
...
const except = compose(provided, not)
var maybe = except(value => {
    return value == null
})
